...
function DoBody({
  ...
  const [selected, setSelected] = useState<
    (typeof DO_RESULTS)[number]["value"] | null
  >(null);
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {isReflecting ? (
          <>
            {DO_RESULTS.map(({ value, label }) => (
              <button
                ...
                className={`min-h-12 w-full rounded-2xl px-4 text-base font-semibold ${choiceButtonClass(selected === value, "primary")}`}
          ...
          <button
            className={`min-h-13 w-full rounded-2xl px-4 text-base font-black text-white shadow-[0_3px_0_var(--color-primary-active)] ${PRIMARY_BUTTON_CLASS} ${showFocus ? "relative z-40 ring-4 ring-primary-border" : ""}`}
...
function SignedInDoPage({ cycleId }: { cycleId: string }) {
  ...
  if (hasError)
    return <LoadFailure message="DOを読み込めませんでした。" onRetry={retry} />;
  if (!isReady || detail === undefined)
    return <LoadingState label="DOを読み込んでいます。" />;
...
function DoGate({ cycleId }: { cycleId: string }) {
  ...
  return isSignedIn ? (
    <SignedInDoPage cycleId={cycleId} />
  ) : (
    <Navigate replace to="/" />
  );
}
