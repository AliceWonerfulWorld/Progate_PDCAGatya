...
import {
  adjustPlanText,
  type PlanAdjustDirection,
} from "../../../convex/lib/planAdjust";
...
function PlanEditor({
  ...
  return (
    <div className="space-y-6">
      <PdcaFlowHeader step="plan" />
      <section className="space-y-3 rounded-3xl border border-primary-border bg-primary-subtle p-5 shadow-sm">
        <p className="text-sm font-medium text-text-subtle">{goalName}</p>
        {isRecovery ? (
          <p className="text-sm font-bold text-attention">リカバリー</p>
        ) : null}
        ...
        {isEditing ? (
          ...
          <p className="rounded-2xl bg-surface p-4 text-xl font-black tracking-tight text-text-strong shadow-sm">
            {text}
          </p>
        )}
      </section>
      ...
      <div className="space-y-3">
        <button
          className={`min-h-13 w-full rounded-2xl px-4 text-base font-black text-white shadow-[0_3px_0_var(--color-primary-active)] ${PRIMARY_BUTTON_CLASS} ${focusNextAction ? "relative z-40 ring-4 ring-primary-border" : ""}`}
        ...
        {isEditing ? null : (
          <>
            <button
              className={`flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl px-4 text-base font-semibold ${SECONDARY_BUTTON_CLASS}`}
...
function SignedInPlanPage({ goalId }: { goalId: string }) {
  ...
  if (hasError)
    return (
      <LoadFailure message="PLANを読み込めませんでした。" onRetry={retry} />
    );
  if (!isReady || detail === undefined || activeCycle === undefined)
    return <LoadingState label="PLANを読み込んでいます。" />;
...
function PlanGate({ goalId }: { goalId: string }) {
  ...
  return isSignedIn ? (
    <SignedInPlanPage goalId={goalId} />
  ) : (
    <Navigate replace to="/" />
  );
}
