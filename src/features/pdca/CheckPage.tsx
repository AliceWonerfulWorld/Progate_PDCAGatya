...
import {
  CheckAckAnimation,
  type CheckAckTone,
} from "../../components/ui/CheckAckAnimation";
...
function CheckBody({
  ...
  return (
    <div className="space-y-6">
      <PdcaFlowHeader step="check" />
      {showFirstLoopGuide ? (
        <FirstLoopGuide
          message="できたかどうかではなく、今日の負荷をそのまま選べば大丈夫。"
          step={4}
          title="やってみた感覚を残そう"
        />
      ) : null}
        ...
        {CHECK_LOADS.map(({ value, label }) => (
          ...
          <div className="relative" key={value}>
            <button
              ...
              className={`min-h-12 w-full rounded-2xl px-4 text-base font-semibold ${choiceButtonClass(checkLoad === value, "primary")}`}
          ...
          <div className="space-y-3">
            {CHECK_REASONS.map(({ value, label }) => (
              <button
                ...
                className={`min-h-12 w-full rounded-2xl px-4 text-base font-semibold ${choiceButtonClass(checkReason === value, "info")}`}
                disabled={isSubmitting}
                key={value}
                onClick={() =>
                  setCheckReason(checkReason === value ? null : value)
                }
                type="button"
              >
                {label}
...
function SignedInCheckPage({ cycleId }: { cycleId: string }) {
  ...
  if (hasError)
    return (
      <LoadFailure message="CHECKを読み込めませんでした。" onRetry={retry} />
    );
  if (!isReady || detail === undefined)
    return <LoadingState label="CHECKを読み込んでいます。" />;
...
function CheckGate({ cycleId }: { cycleId: string }) {
  ...
  return isSignedIn ? (
    <SignedInCheckPage cycleId={cycleId} />
  ) : (
    <Navigate replace to="/" />
  );
}
