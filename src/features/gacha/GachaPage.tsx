  ...
  return (
    ...
    >
      ...
      <div className="space-y-1.5 p-4">
        ...
        {gacha.description ? (
          <p className="text-sm text-text-muted">{gacha.description}</p>
        ) : null}
        <p className="truncate text-xs text-text-subtle">
          {gacha.characterNames.join("、")}
        </p>
        <p className="flex items-center gap-1 text-xs font-semibold text-primary">
          <Clock aria-hidden="true" className="size-3.5" />{" "}
          {formatRemainingTime(gacha.endAt)}
        </p>
      </div>
    </button>
  );
...
function SignedInGacha() {
  ...
  const [selectedGacha, setSelectedGacha] = useState<GachaBannerInfo | null>(
    null,
  );
  ...
  if (hasError)
    return (
      <LoadFailure message="ガチャを読み込めませんでした。" onRetry={retry} />
    );
  ...
  if (result) {
    return (
      <GachaResultView
        ...
        result={result}
        secondaryLinks={[
          ...(result.availableGachaDraws <= 0 && goalId
            ? [{ label: "もう1周する", to: `/pdca/plan/${goalId}` }]
            : []),
          ...(goalId
            ? [{ label: "Goalを確認する", to: `/goal/${goalId}` }]
            : []),
          { label: "Goal一覧を見る", to: "/goals" },
    ...
    <div className="space-y-8 text-center">
      <section className="space-y-3">
        ...
        <p className="flex items-center justify-center gap-1 text-xs font-semibold text-primary">
          <Clock aria-hidden="true" className="size-3.5" />{" "}
          {formatRemainingTime(selectedGacha.endAt)}
        </p>
...
function GuestGacha() {
  ...
  const gachaRates = useQuery(api.gachas.getActiveGachaRates, {
    key: "standard",
  });
  const [isDrawing, setIsDrawing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  async function handleDraw() {
    ...
    const candidates = activeCharacters.filter(
  ...
  return (
    <div className="space-y-8 text-center">
      ...
      <button
        ...
        type="button"
      >
        {activeCharacters === undefined || gachaRates === undefined
          ? "準備しています…"
          : "1回回す"}
      </button>
    </div>
